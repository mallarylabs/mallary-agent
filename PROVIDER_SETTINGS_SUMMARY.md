...
| Instagram | `instagram` | `post_type`, `shareToFeed` for Reel feed placement, `trialParams.graduationStrategy` for Trial Reels, and Facebook Login-only `isPaidPartnership` plus `brandedContentSponsors`. Media `thumbnail_url` sets video/Reels covers. |
